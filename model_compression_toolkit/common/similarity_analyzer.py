...
def compute_acs(float_tensor: np.ndarray, fxp_tensor: np.ndarray, eps: float = 1e-8) -> float:
    """
    Compute the similarity between two tensor using cosine similarity.
    The returned values is between 0 to 1: the smaller returned value,
    the greater similarity there is between the two tensors.
    ...
    # compute the L2 norm.
    float_norm = np.linalg.norm(float_tensor, axis=-1, ord=2)
    fxp_norm = np.linalg.norm(fxp_tensor, axis=-1, ord=2)
